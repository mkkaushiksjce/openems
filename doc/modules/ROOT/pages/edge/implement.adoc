...
<3> The *id* configuration parameter sets the OpenEMS Component-ID (see xref:coreconcepts.adoc#_channel[Channel Adress]). _Note_: A *default* ID 'meter0' is defined. It is good practice to define such an ID here, as it simplifies configuration in the UI.
